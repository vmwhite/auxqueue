test_that("check A_k matrix", {
  ...
  ## Error in Green text transition from (i, 2) to (i,1) should be mu_A not mu_A*p
  ...
  A_2 <-    matrix(c(3*mu_P*p,0,0,0,0,
                     ...
                     0, 0,0,0,2*mu_P)
                   , nrow = 5, ncol = 5, byrow = TRUE)
  ## Error in Green text transition from (i, 3) to (i-2,4) should be 2*mu_P*q not 2*mu_P*calc_alpha_i(p,1)
  ...
  A_1 <-    matrix(c(-(lam+2*mu_P),0,0,
                   ...
                   , nrow = 3, ncol = 3, byrow = TRUE)
  A_2 <-    matrix(c(2*mu_P*p,0,0,
                     0, mu_P*p+mu_A*q,  0,
                     0,0,mu_P)
                   , nrow = 3, ncol = 3, byrow = TRUE)
  A_3 <-    matrix(c(0,2*mu_P*q,0,
                     0, 0,mu_P*q,
                     0,0,0)
                   , nrow = 3, ncol = 3, byrow = TRUE)
